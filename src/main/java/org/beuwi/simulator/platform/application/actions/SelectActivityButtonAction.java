...
public class SelectActivityButtonAction
{
	...
	{
		if (SideBarManager.isHided(index))
		{
			HideSideBarAction.update(index);
			SideBarManager.setHided(index, true);
		}
		else
		{
			ShowSideBarAction.update(index);
			SideBarManager.setHided(index, false);
		}
	}
}