...
import org.beuwi.msgbots.view.app.tabs.BotListTab;
...
public class RefreshAllBotsAction implements Executor {
	...
	public void execute() {
		...
		if (status) {
			...
			File[] folders = SharedValues.getFile("file.botFolder").listFiles(File::isDirectory);
			...
			for (int i = 0 ; i < length ; i ++) {
				...
				if (Project.check(folder)) {
					...
					// 새로 등록된 프로젝트라면
						...
						projects.add(project);
						GUIManager.register(project); // GUIManager 등록
					}
