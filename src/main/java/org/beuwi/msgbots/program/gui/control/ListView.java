...
public class ListView<T> extends javafx.scene.control.ListView<T> {
	...
	public ListView() {
		...
		// 스킨이 생성 돼야, Lookup 사용 가능함
		/* skinProperty().addListener(change -> {
			if (getSkin() != null) {
				ListViewSkin skin = (ListViewSkin) getSkin();
				VirtualFlow flow = (VirtualFlow) skin.getChildren().get(0);
				List<Node> list = flow.getChildrenUnmodifiable();
				// VirtualFlow, Group, VirtualScrollBar, VirtualScrollBar
				Set<Node> set = lookupAll(".scroll-bar");
				for (Node node : set) {
					node.setVisible(false);
					// 여기서 가시성을 지정해도, 추후 생성될 때 내부에서 다시 가시성을 온으로 하는 경우가 있음
					// 따라서 ListViewSkin 내부에서, 가시성을 true값으로 지정할 때마다 감지해서 안보이도록 지정
					node.visibleProperty().addListener(change1 -> {
						node.setVisible(false);
					});
					// 스크롤이 안되도록 막음
	...
	public Node findById(String name) {
		for (T item : getItems()) {
